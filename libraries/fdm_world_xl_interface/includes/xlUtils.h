...
bool kXlUtils::checkDim(LPXLOPER12 in, int row, int col, string* error) {
  if (row < 0 || (int)getRows(in) <= row || col < 0 ||
      (int)getCols(in) <= col) {
...
bool kXlUtils::getDbl(LPXLOPER12 in, int row, int col, double& num,
                      string* error) {
...
bool kXlUtils::getInt(LPXLOPER12 in, int row, int col, int& i, string* error) {
  double d;
...
bool kXlUtils::getStr(LPXLOPER12 in, int row, int col, string& str,
                      string* error) {
  if (!checkDim(in, row, col, error)) return false;
...
void kXlUtils::setDbl(int row, int col, double num, LPXLOPER12 out) {
...
LPXLOPER12
kXlUtils::setError(const string& error) {
  //	declare, resize, fill
...
bool kXlUtils::getVector(LPXLOPER12 in, mVector<double>& out) {
  ...
  for (size_t i = 0; i < m; ++i) {
    ...
    if (d == numeric_limits<double>::infinity()) return false;
    out((int)i) = d;
  }
  return true;
}
...
void kXlUtils::setMatrix(const mMatrix<double>& in, LPXLOPER12 out) {
  ...
  resize(out, m, n);
  for (size_t i = 0; i < m; ++i)
    for (size_t j = 0; j < n; ++j) setNum(out, in((int)i, (int)j), i, j);
}
