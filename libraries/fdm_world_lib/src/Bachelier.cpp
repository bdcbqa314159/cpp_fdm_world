...
bool Bachelier::fdRunner(const double s0, const double r, const double mu,
                          ...
                          const double theta, const int wind,
                          const double numStd, const int numT, const int numS,
                          const bool update, const int numPr, double& res0,
                          mVector<double>& s, mVector<double>& res,
                          std::string& error) {
  //	helps
  int h, i, p;
  ...
  for (i = 0; i < nums; ++i) {
    if (smooth == 0 || i == 0 || i == nums - 1) {
      if (dig)
        res(i) = 0.5 * (Inlines::sign(s(i) - strike) + 1.0);
      else
      ...
      xu = 0.5 * (s(i) + s(i + 1));
      if (dig)
        res(i) = kFiniteDifference::smoothDigital(xl, xu, strike);
      else
